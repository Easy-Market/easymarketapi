...
class auth
{
    public function signup($conn, $email, $phoneNum, $fname, $lname, $pass1, $pass2)
    {
        ...
        if ($conn->affected_rows > 0) { //check if user exists
            ...
            if ($pass1 == $pass2) { //check if passwords match
                ...
                $newUser = $conn->prepare("INSERT INTO users(us_email, us_phone, us_password, us_fname, us_lname) VALUES(?, ?, ?, ?, ?)");
                $newUser->bind_param("sssss", $email, $phoneNum, $password, $fname, $lname);
                if ($newUser->execute()) {
                    $success['code'] = 200;
    ...
    public function signupVendor($conn, $email, $phoneNum, $fname, $lname, $businessname, $cansell, $displaymode, $pass1, $pass2, $vendorurl)
    {
        $conn->query("SELECT * FROM users WHERE us_email = '$email' OR us_phone = '$phoneNum'");
        if ($conn->affected_rows > 0) { //check if user exists
            $userExists['code'] = 419;
